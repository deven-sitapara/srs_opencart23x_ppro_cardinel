...
class ModelPaymentSrsPpproThreed extends Model {
  public function getMethod($address, $total) {
    ...
    if ($this->config->get('srs_pppro_threed_total') > 0 && $this->config->get('srs_pppro_threed_total') > $total) {
      $status = false;
    } elseif (!$this->config->get('srs_pppro_threed_geo_zone_id')) {
      $status = true;
    ...
    if ($status) {
      $method_data = array(
        'code'       => 'srs_pppro_threed',
        'title'      => $this->language->get('text_title'),
        'terms'      => '',
        'sort_order' => $this->config->get('srs_pppro_threed_sort_order')
      );
    }
